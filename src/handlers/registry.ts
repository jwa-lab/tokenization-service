...
import { RegistryContract, RegistryStorage } from "../contracts/registry.types";
...
export const registryHandlers: NatsHandler[] = [
    [
        ...
        async (err: NatsError | null, msg: Msg): Promise<void> => {
            const registryContract = await getContract<RegistryContract>(
    ...
    [
        ...
        async (err: NatsError | null, msg: Msg): Promise<void> => {
            ...
            const storage = await registryContract.storage<RegistryStorage>();

            try {
                const collectible = (await storage.registry.get(
                    msg.data
